<?php
/**
 * TBB Vault — Section hub (replaces theme template-sub-child.php when assigned).
 */
...
<div id="primary" class="content-area primary tbb-vault tbb-vault--section">
	<main id="main" class="site-main">
		<?php
		while (have_posts()) {
			the_post();
			$post = get_post();
			do_action('tbb_vault_before_section', $post);
			if (!$can_view) {
				do_action('tbb_vault_memberpress_gate', $post);
				break;
			}
			if (tbb_vault_feature_enabled('show_back_to_parent') && $post instanceof WP_Post && (int) $post->post_parent > 0) {
				$p = get_post($post->post_parent);
				if ($p instanceof WP_Post) {
					echo '<p class="tbb-vault-back"><a href="' . esc_url(get_permalink($p)) . '">← ' . esc_html($p->post_title) . '</a></p>';
				}
			}
			...
			do_action('tbb_vault_after_section', get_post());
		}
		?>
	</main>
</div>
